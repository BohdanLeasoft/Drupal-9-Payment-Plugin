...
class AbstractPayment extends PaymentOffsiteForm
{
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state)
  {
    ...
    $paymentForm = $this->entity->getPaymentGateway()->getPlugin()->prepareForm($form);
    if ($paymentForm) {
      return $paymentForm;
    }
  ...
  public function startTransaction(array &$form, FormStateInterface $form_state)
  {
    ...
    $birthdate = OrderController::getBirthdate($values) ? new Birthdate(OrderController::getBirthdate($values)) : null;
    $gender = OrderController::getCustomerGender($values, $form_state);

    $customer = $this->builderRedefiner->getCustomerData($payment, $birthdate, $gender);

    $order = $this->builderRedefiner->createOrder($payment, $this->client, $customer, $this->paymentMethod, $issuerId, $verifiedTerms);
